        ...
        <div className="flex flex-col gap-10 items-center justify-center w-full">
            ...
            <div className="flex flex-row max-md:flex-col flex-wrap justify-center items-start gap-5">
                {sortedDeals.map(deal => (
                    <Link to={`/dashboard/${deal.id}`} key={deal.id} className="flex flex-col justify-start items-start w-72 max-lg:w-64 gap-2 rounded-xl border border-mansa-blue hover:border-dark-blue">
                        ...
                        <p className="text-dark-blue text-base px-5"><span className="font-semibold">Deadline: </span> {deal.dead_line} </p>
                    </Link>
                ))}
            </div>
        </div>
