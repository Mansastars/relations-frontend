...
export default function ConfirmEmail({userEmail, onClose, show}) {
  ...
  const resendMail = async () => {
    ...
    try{
      ...
      setIsSuccess(true);
      setSuccessMessage("Success! Your confirmation email has been resent. Please check your inbox.")
    } catch (error){
      console.log(error);
      setSuccessMessage("")
      setIsSuccess(false);
      ...
      <Modal show={show} onClose={onClose}>
        ...
        <Modal.Body>
          <div className="flex flex-col gap-5 items-center">
            {!isError ? <img src={email} alt="Email icon" width={100} className=" max-sm:w-14" /> : <img src={x_button} alt="Error Icon" width={100} className="max-sm:w-14" />}
            ...
            {!isError && !isLoading && <p className="text-center text-dark-blue max-sm:text-base">
                We have sent an email to <span className="text-mansa-blue">{userEmail}</span> to
                confirm the validity of your email address. After receicing the email, please follow the
