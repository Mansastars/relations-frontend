    ...
    const fetchOnGoingReviews = async () => {
      ...
      try {
        const response = await api.get(
          `deals/review-contacts/${currentDealId}`
        );
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          setOnGoingReviews(
            OnGoingReviews.filter((OnGoingReview) => OnGoingReview.id !== id)
          );
          Swal.fire(
            "Deleted!",
          ...
          <div style={{ overflowY: "auto", maxHeight: "calc(100vh - 100px)" }}>
            {OnGoingReviews.map((OnGoingReview) => (
              <div
                ...
                onDoubleClick={() => handleEdit(OnGoingReview.id)}
              ...
              >
                ...
                >
                  <div className="flex justify-between w-full">
                    ...
                    <button
                      onClick={() => handleDelete(OnGoingReview.id)}
                  ...
                  <p className="text-sm text-white">
                    {OnGoingReview.organization_name
                      ? OnGoingReview.organization_name.length > 15
                        ? OnGoingReview.organization_name.substring(0, 15) +
                          "..."
                        : OnGoingReview.organization_name
                      : "No company entered"}
                  </p>
                ...
                <div className="flex flex-col gap-1 p-2 items-start bg-light-grey rounded-2xl">
                  <div>
                    <p className="text-xs font-semibold">
                      ...
                      {OnGoingReview.deal_size
                        ? OnGoingReview.deal_size.length > 15
                          ? "$" +
                            addCommasToNumber(
                              OnGoingReview.deal_size.substring(0, 15)
                            ) +
                            "..."
                    ...
                    <p className="text-xs">
                      {OnGoingReview.email
                        ? truncateEmail(OnGoingReview.email, 20)
                        : "No email entered"}
                    </p>
                    <p className="text-xs">
                  ...
                  <div className="flex flex-col justify-center items-start">
                    <p className="text-xs text-wrap">
                      {OnGoingReview.notes.length > 20
                        ? OnGoingReview.notes.substring(0, 20) + "..."
                        : OnGoingReview.notes}
                    </p>
                  </div>
                </div>
