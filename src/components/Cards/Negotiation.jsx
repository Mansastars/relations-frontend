    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          setNegotiations(
            negotiations.filter((negotiation) => negotiation.id !== id)
          );
          Swal.fire(
            "Deleted!",
    ...
    <>
      {negotiations.length === 0 || undefined ? (
        ...
        negotiations.map((negotiation) => (
          <div
            key={negotiation.id}
            onDoubleClick={() => handleEdit(negotiation.id)}
          ...
          >
            ...
            >
              <div className="flex justify-between w-full">
                <p className="font-extrabold text-sm text-white">
                  {`${negotiation.first_name} ${negotiation.last_name}`.length >
                  13
              ...
              <p className="text-sm text-white">
                {negotiation.organization_name
                  ? negotiation.organization_name.length > 15
                    ? negotiation.organization_name.substring(0, 15) + "..."
                    : negotiation.organization_name
                  : "No company entered"}
              </p>
            </div>
            <div className="flex flex-col gap-1 p-2 items-start bg-light-grey rounded-2xl">
              <div>
                ...
                <p className="text-xs">
                  {negotiation.email
                    ? truncateEmail(negotiation.email, 20)
                    : "No email entered"}
                </p>
                <p className="text-xs">
              ...
              <div className="flex flex-col justify-center items-start">
                <p className="text-xs text-wrap">
                  {negotiation.notes.length > 20
                    ? negotiation.notes.substring(0, 20) + "..."
                    : negotiation.notes}
                </p>
              </div>
            </div>
