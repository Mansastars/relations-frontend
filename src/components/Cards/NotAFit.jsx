    ...
    <>
      {notAFits.length === 0 || undefined ? (
        ...
        notAFits.map((notAFit) => (
          <div
            key={notAFit.id}
            onDoubleClick={() => handleEdit(notAFit.id)}
          ...
          >
            ...
            >
              <div className="flex justify-between w-full">
                <p className="font-extrabold text-sm text-white">
                  {`${notAFit.first_name} ${notAFit.last_name}`.length > 13
                ...
                <button
                  onClick={() => handleDelete(notAFit.id)}
              ...
              <p className="text-sm text-white">
                {notAFit.organization_name
                  ? notAFit.organization_name.length > 15
                    ? notAFit.organization_name.substring(0, 15) + "..."
                    : notAFit.organization_name
                  : "No company entered"}
              </p>
            </div>
            <div className="flex flex-col gap-1 p-2 items-start bg-light-grey rounded-2xl">
              <div>
                <p className="text-xs font-semibold">
                  ...
                  {notAFit.deal_size
                    ? notAFit.deal_size.length > 15
                      ? "$" +
                        addCommasToNumber(notAFit.deal_size.substring(0, 15)) +
                        "..."
                ...
                <p className="text-xs">
                  {notAFit.email
                    ? truncateEmail(notAFit.email, 20)
                    : "No email entered"}
                </p>
                <p className="text-xs">
              ...
              <div className="flex flex-col justify-center items-start">
                <p className="text-xs text-wrap">
                  {notAFit.notes.length > 20
                    ? notAFit.notes.substring(0, 20) + "..."
                    : notAFit.notes}
                </p>
              </div>
            </div>
