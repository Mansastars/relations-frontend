                    ...
                    <div style={{overflowY: 'auto', maxHeight: 'calc(100vh - 100px)'}}>
                        {
                            prospects.map(prospect => (
                                <div key={prospect.id} onDoubleClick={() => handleEdit(prospect.id)} title="Double click to edit contact" className="flex flex-col rounded-2xl mb-2 h-44 cursor-pointer" style={{...BorderStyle, minWidth: '165px'}}>
                                    <div className="flex flex-col p-2 rounded-t-2xl border-b-dark-blue items-start" style={{ background: borderColour }}>
                                        <div className="flex justify-between w-full">
                                            <p className="font-extrabold text-sm text-white">
                                                {`${prospect.title} ${prospect.first_name} ${prospect.last_name}`.length > 13 ? `${prospect.title} ${prospect.first_name} ${prospect.last_name}`.substring(0, 10) + '...' : `${prospect.title} ${prospect.first_name} ${prospect.last_name}`}
                                            </p>
                                    ...
                                    <div className="flex flex-col gap-1 p-2 items-start bg-light-grey rounded-2xl">
                                        <div>
                                            <p className="text-xs font-semibold">
                                                Deal Size: {prospect.deal_size ? (prospect.deal_size.length > 15 ? '$' + addCommasToNumber(prospect.deal_size.substring(0, 15)) + '...' : '$' + addCommasToNumber(prospect.deal_size)) : '$0'}
                                            </p>
                                            <p className="text-xs font-semibold">
                                                Meeting: {prospect.meeting_date ? new Date(prospect.meeting_date).toLocaleString() : 'No meeting date entered'}
                                            </p>
                                            <p className="text-xs">
                                                {prospect.email ?  truncateEmail(prospect.email, 20) : 'No email entered'}
                                            </p>
                                            <p className="text-xs">
