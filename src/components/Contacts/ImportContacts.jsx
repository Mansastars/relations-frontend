    ...
    if (acceptedFiles.length > 0) {
      ...
      reader.onload = async () => {
        ...
        try {
          ...
          const fieldNames = headerRow.map((value) =>
            value ? value.trim().toLowerCase() : ""
          ); // Convert to lowercase

          // Check if any required field is present in the header row
          const hasRequiredField = fieldNames.some((fieldName) =>
    ...
    <div className="px-3 flex flex-col my-5 w-full justify-center items-center overflow-x-auto">
        ...
        <div {...getRootProps({ style })}>
          ...
          <div className=" flex flex-col gap-2">
            <p className="text-lg text-center">
              ...
              <span className="text-mansa-blue underline cursor-pointer">
                browse
              </span>
            </p>
            <em className="text-base text-center">
      ...
      {extractedData.length === 0 ? (
        <div className="w-full my-10">
          <h2 className="text-2xl font-bold mb-4 text-left self-start">
            Sample CSV File
          </h2>
          <img src={sampleCsv} />
        </div>
